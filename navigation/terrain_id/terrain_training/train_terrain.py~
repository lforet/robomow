...
def predict_class(img):
	model = pickle.load( open( "robomow_ai.mdl", "rb" ) )
	features = find_features(img)
	classID = classify(model, features)
	print "predicted classID:", classID
	eg.msgbox("predicted classID:"+str(classID))
...
if __name__=="__main__":
	...
	while loop == 1:
		...
		reply =	eg.buttonbox(msg='Classify Image', title='Robomow GUI', choices=('Mowable', 'Non-Mowable', 'Next Frame', 'Predict', 'Retrain AI' ,'Quit'), image='temp.png', root=None)
