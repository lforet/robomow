...
CORBET ET AL., p.353.  THIS IS THE CASE FOR OpenSUSE.
-----------------------------------------------------------------------------*/
...
-----------------------------------------------------------------------------*/
struct file_operations easysnd_fops =
  {
  .owner =   THIS_MODULE,
  .open =    easysnd_open,
...
if ( ! ( pusb_interface = \
                    usb_find_interface(&easycap_usb_driver, iminor(inode) )))
  {
...
/*vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv*/
#else
for ( rc = 0;  rc < video_device_many;  rc++ )
  {
  if ((struct video_device *)NULL != ( pvideo_device = pvideo_array[rc] ))
    {
...
if ( 0 == rc ) { JOT(8,"wakeup_device() OK\n"); }
else
  {
...
rc = setup_stk(peasycap->pusb_device);
if ( 0 == rc ) { JOT(4,"setup_stk() OK\n"); }
else
  {
  SAY("ERROR: setup_stk(.,0) returned %i\n",rc);
  return(-EFAULT);
  }
rc = setup_saa(peasycap->pusb_device);
if ( 0 == rc ) { JOT(4,"setup_saa() OK\n"); }
else
  {
  SAY("ERROR: setup_saa(.,0) returned %i\n",rc);
  return(-EFAULT);
  }
rc = check_saa(peasycap->pusb_device);
if ( 0 == rc )       { JOT(4,"check_saa() OK\n"); }
...
for ( i = 0; i < 180; i++ ) { peasycap->merit[i] = 0; }
peasycap->video_eof = 0;
peasycap->audio_eof = 0;
do_gettimeofday( &peasycap->timeval7 );
/*-----------------------------------------------------------------------------
RESTORE INPUT, STANDARD AND FORMAT
-----------------------------------------------------------------------------*/
peasycap->input = -1;
peasycap->standard_offset = -1;
peasycap->format_offset = -1;

rc = newinput( peasycap, input );

if ( 0 == rc ) JOT(4,"restored input, standard and format\n");
else
  {
  SAY("ERROR: newinput(.,%i) returned %i\n",rc,input);
  return(-EFAULT);
  }
...
JOT(8,"%i=input sought\n", input);

if (( 0 > input ) && ( 6 <= input )) return(-ENOENT);
inputnow = peasycap->input;
if ( input == inputnow ) return(0);
...
if ( input == peasycap->inputset[input].input )
  ...
  if ( off != peasycap->standard_offset )
    ...
    if ( 0 != rc )
      {
      SAY("ERROR: adjust_standard() returned %i\n",rc);
      return(-EFAULT);
      }
    }
  ...
  if ( off != peasycap->format_offset )
    ...
    if ( 0 > rc )
      {
      SAY("ERROR: adjust_format() returned %i\n",rc);
      return(-EFAULT);
      }
    }
  ...
  if ( mood != peasycap->brightness )
    {
    if ( 0 != ( rc = adjust_brightness( peasycap, mood )))
      {
      SAY("ERROR: adjust_brightness returned %i\n",rc);
      return(-EFAULT);
      }
    }
  ...
  if ( mood != peasycap->contrast )
    {
    if ( 0 != ( rc = adjust_contrast( peasycap, mood )))
      {
      SAY("ERROR: adjust_contrast returned %i\n",rc);
      return(-EFAULT);
      }
    }
  ...
  if ( mood != peasycap->saturation )
    {
    if ( 0 != ( rc = adjust_saturation( peasycap, mood )))
      {
      SAY("ERROR: adjust_saturation returned %i\n",rc);
      return(-EFAULT);
      }
    }
  ...
  if ( mood != peasycap->hue )
    {
    if ( 0 != ( rc = adjust_hue( peasycap, mood )))
      {
      SAY("ERROR: adjust_hue returned %i\n",rc);
      return(-EFAULT);
      }
    }
  }
...
if ( true == resubmit ) submit_video_urbs(peasycap);
...
if ( ! peasycap->video_isoc_streaming )
  ...
  if ( 0 != ready_saa( peasycap->pusb_device ) )
    {
    JOT(8,"not ready to capture: 0x%02X ... continuing anyway\n", \
  ...
  list_for_each( plist_head, ( peasycap->purb_video_head ))
    ...
    if ( NULL != pdata_urb )
      ...
      if ( NULL != ( purb = pdata_urb->purb ))
        ...
        purb->transfer_buffer_length = peasycap->video_isoc_buffer_size;
        ...
        for ( j = 0;  j < peasycap->video_isoc_framesperdesc; j++ )
          ...
          purb->iso_frame_desc[j].length = peasycap->video_isoc_maxframesize;
          }

        if ( 0 != ( rc = usb_submit_urb( purb, GFP_KERNEL ) ))
          {
          isbad++;
          SAY("ERROR: usb_submit_urb() failed for urb with rc:\n");
          switch (rc)
            {
            ...
            default: { SAY("unknown error code %i.\n", rc ); break; }
            }
          }
        else { m++; }
        }
  ...
  if ( isbad )
    ...
    list_for_each( plist_head, ( peasycap->purb_video_head ))
      ...
      if ( NULL != pdata_urb )
        {
        if ( NULL != ( purb = pdata_urb->purb )) usb_kill_urb( purb );
        }
      }
...
if ( NULL == ( peasycap = (struct easycap *)file->private_data ))
  {
  SAY("ERROR:  peasycap is NULL.\n");
...
for ( i = 0;  i < video_device_many;  i++ )
  {
  if ((struct video_device *)NULL != ( pvideo_device = pvideo_array[i] ))
    {
    if ( pvd->minor == pvideo_device->minor )
      {
      if ( (struct easycap *)NULL == ( peasycap = \
                     (struct easycap *)video_get_drvdata(pvideo_device) ))
        {
        SAY("ERROR:  peasycap is NULL\n");
        SAY("ending unsuccessfully\n");
        return(-EFAULT);
      ...
      JOT(4,"freeing video_device structure: /dev/video%i\n", i );
...
/*---------------------------------------------------------------------------*/
  ...
  list_for_each_safe( plist_head, plist_next, peasycap->purb_video_head )
    ...
    if ((struct data_urb *)NULL != pdata_urb )
      ...
      peasycap->allocation_video_struct -= sizeof(struct data_urb);
      m++;
      }
    }
...
for ( k = 0;  k < VIDEO_ISOC_BUFFER_MANY;  k++ )
  ...
  if ((void *)NULL != peasycap->video_isoc_buffer[k].pgo )
    {
    free_pages((unsigned long)( peasycap->video_isoc_buffer[k].pgo ), \
                                                     VIDEO_ISOC_ORDER );
...
for ( k = 0;  k < FIELD_BUFFER_MANY;  k++ )
  ...
  for ( m = 0;  m < FIELD_BUFFER_SIZE/PAGE_SIZE;  m++ )
    ...
    if ((void *)NULL != peasycap->field_buffer[k][m].pgo )
      {
      free_page((unsigned long)( peasycap->field_buffer[k][m].pgo ));
...
for ( k = 0;  k < FRAME_BUFFER_MANY;  k++ )
  ...
  for ( m = 0;  m < FRAME_BUFFER_SIZE/PAGE_SIZE;  m++ )
    ...
    if ((void *)NULL != peasycap->frame_buffer[k][m].pgo )
      {
      free_page((unsigned long)( peasycap->frame_buffer[k][m].pgo ));
...
/*---------------------------------------------------------------------------*/
  ...
  list_for_each_safe( plist_head, plist_next, peasycap->purb_audio_head )
    ...
    if ((struct data_urb *)NULL != pdata_urb )
      {
      kfree( pdata_urb );  pdata_urb = (struct data_urb *)NULL;
      peasycap->allocation_audio_struct -= sizeof(struct data_urb);
      m++;
      }
    }
...
for ( k = 0;  k < AUDIO_ISOC_BUFFER_MANY;  k++ )
  ...
  if ((void *)NULL != peasycap->audio_isoc_buffer[k].pgo )
    {
    free_pages((unsigned long)( peasycap->audio_isoc_buffer[k].pgo ), \
                                                     AUDIO_ISOC_ORDER );
...
if ( 0 == easycap_dqbuf( peasycap, 0 )) return( POLLIN | POLLRDNORM );
else                                    return( POLLERR );
...
-----------------------------------------------------------------------------*/
miss = 0;
if (down_interruptible(&( peasycap->sem_mmap_video[0]))) return(-ERESTARTSYS);

while (( peasycap->field_read == peasycap->field_fill ) || \
   ( 0 != \
   ( 0xFF00 & peasycap->field_buffer[ peasycap->field_read ][0].kount )) || \
   ( 0 != \
   ( 0x00FF & peasycap->field_buffer[ peasycap->field_read ][0].kount )))
  {
  up( &( peasycap->sem_mmap_video[0] ));

  if ( mode ) return(-EAGAIN);
  ...
  if ( 0 != ( wait_event_interruptible( peasycap->wq_video, \
   ...
   ( 0x00FF & peasycap->field_buffer[ peasycap->field_read ][0].kount )))))))
    {
    SAY("aborted by signal ... returning -EIO\n");
    return( -EIO );
    }
  if ( peasycap->video_idle )
    {
    JOT(8,"%i=peasycap->video_idle ... returning -EAGAIN\n", \
                                                       peasycap->video_idle);
    return( -EAGAIN );
    }
...
if ( true == peasycap->offerfields )
  ...
  if ( 0x01 & easycap_standard[ peasycap->standard_offset ].mask )
    {
    peasycap->frame_buffer[peasycap->frame_read][0].kount = V4L2_FIELD_BOTTOM;
    }
  else
    {
    peasycap->frame_buffer[peasycap->frame_read][0].kount = V4L2_FIELD_TOP;
    }
...
THE BOOLEAN PARAMETER offerfields IS true ONLY WHEN THE USER PROGRAM CHOOSES
THE OPTION V4L2_FIELD_ALTERNATE.  NO USERSPACE PROGRAM TESTED TO DATE HAS DONE
THIS.  BUGS ARE LIKELY.
-----------------------------------------------------------------------------*/
int
...
int bytesperpixel, multiplier, much, more, over, rump, caches;
__u8 mask,margin;
bool odd, isuy, decimatepixel, offerfields, badinput;

badinput = false;

JOT(8,"=====  parity %i, input 0x%02X, field buffer %i --> frame buffer %i\n",\
              peasycap->field_buffer[ peasycap->field_read ][0].kount, \
              peasycap->field_buffer[ peasycap->field_read ][0].input, \
              peasycap->field_read, peasycap->frame_fill );
JOT(8,"=====  %i=bytesperpixel\n", peasycap->bytesperpixel );
if ( true == peasycap->offerfields ) JOT(8,"===== offerfields\n");
...
if (( 2 != bytesperpixel ) && ( 3 != bytesperpixel ) && ( 4 != bytesperpixel ))
  {
  SAY("MISTAKE: %i=bytesperpixel\n", bytesperpixel );
  return(-EFAULT);
  }
if ( true == decimatepixel ) multiplier = 2;
else                         multiplier = 1;

w2 = 2 * multiplier * peasycap->width;
w3 = bytesperpixel * multiplier * peasycap->width;
wz = multiplier * peasycap->height * multiplier * peasycap->width;
...
if ( peasycap->field_buffer[kex][0].kount ) odd = true;
else                                        odd = false;
if ( peasycap->field_buffer[kex][0].input != ( 0x08 | peasycap->input ))
  {
  JOT(8,"ERROR: 0x%02X=->field_buffer[%i][0].input, 0x%02X=(0x08|->input)\n", \
              peasycap->field_buffer[kex][0].input,kex,(0x08|peasycap->input));
  return(-ENOENT);
  }
peasycap->frame_buffer[kad][0].input = peasycap->field_buffer[kex][0].input;
...
-----------------------------------------------------------------------------*/
  if ( false == decimatepixel )
    ...
    do
      ...
      if ( rex < much ) much = rex;
      rump = 0;

      if ( much % 2 ) { SAY("MISTAKE: much is odd\n"); return(-EFAULT); }
      ...
      if ( 1 < bytesperpixel )
        ...
        if ( much == rex )
          ...
          if (( mex + 1 ) < FIELD_BUFFER_SIZE/PAGE_SIZE )
            {
            margin = *( (__u8 *)( peasycap->field_buffer[kex][mex + 1].pgo ) );
            }
          else mask |= 0x08;
          }
        }
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
        ...
        {
        SAY("MISTAKE: %i=bytesperpixel\n", bytesperpixel); return(-EFAULT);
        }
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
      if ( rump ) caches++;
      if ( true == badinput )
        {
        JOT(8,"ERROR: 0x%02X=->field_buffer[%i][%i].input, " \
                                                  "0x%02X=(0x08|->input)\n", \
                             peasycap->field_buffer[kex][mex].input,kex,mex, \
                                                       (0x08|peasycap->input));
        }

      if ( 0 > ( redaub( peasycap, pad, pex, much, more, mask, margin, isuy )))
        {
      ...
      if ( ! rex )
        {
        mex++;  pex = peasycap->field_buffer[kex][mex].pgo;  rex = PAGE_SIZE;
        if ( peasycap->field_buffer[kex][mex].input != \
                                 ( 0x08 | peasycap->input )) badinput = true;
        }
      pad  += more;  rad -= more;
      if ( ! rad )
        {
        mad++;  pad = peasycap->frame_buffer[kad][mad].pgo;  rad = PAGE_SIZE;
        if ( rump ) { pad += rump;  rad -= rump; }
        }
      } while ( over );
/*-----------------------------------------------------------------------------
...
if ( cz  !=  c2 ) SAY("ERROR: discrepancy %i in bytes read\n",c2-cz);
...
if ( false == decimatepixel )
  ...
  if ( false == odd )
    {
    if ( bytesperpixel*cz != ( 4 * c3 )) \
                            SAY("ERROR: discrepancy %i in bytes written\n", \
                                                   (2*c3)-(bytesperpixel*cz));
    }
  else
    {
    if ( 0 != c3 ) SAY("ERROR: discrepancy %i in bytes written\n", c3);
    }
  }
if ( rump ) SAY("ERROR: undischarged cache at end of line in frame buffer\n");

JOT(8,"===== field2frame(): %i bytes --> %i bytes (incl skip)\n", c2, c3);
JOT(8,"===== field2frame(): %i=mad  %i=rad\n", mad, rad );

if (true == odd) JOT(8,"+++++ field2frame():  frame buffer %i is full\n",kad);
...
if ( peasycap->timeval6.tv_sec )
  ...
  below = ((long long int)( 1000000 )) * \
          ((long long int)( timeval.tv_sec  - peasycap->timeval6.tv_sec )) + \
...
if ((( 0 <= above ) && ( 0 <= below )) || \
  ...
  {
  if ( 0 > above ) above = -above;
  if ( 0 > below ) below = -below;
  sdr.remainder = (unsigned long long int) do_div( above, below );
  sdr.quotient  = -( (long long int) above );
  }
return(sdr);
...
much BYTES ARE READ, AT LEAST (bytesperpixel * much)/2 BYTES ARE WRITTEN IF
THERE IS NO DECIMATION, HALF THIS AMOUNT IF THERE IS DECIMATION.

mask IS ZERO WHEN NO SPECIAL BEHAVIOUR REQUIRED. OTHERWISE IT IS SET THUS:
  0x03 & mask = number of bytes to be written to cache instead of frame buffer
...
if ( ! bu[255] )
  ...
  for ( j = 0; j < 112; j++ )
    {
    s32 = (0xFF00 & (453 * j)) >> 8; bu[j + 128] =  s32; bu[127 - j] = - s32;
    s32 = (0xFF00 & (359 * j)) >> 8; rv[j + 128] =  s32; rv[127 - j] = - s32;
    s32 = (0xFF00 & ( 88 * j)) >> 8; gu[j + 128] =  s32; gu[127 - j] = - s32;
    s32 = (0xFF00 & (183 * j)) >> 8; gv[j + 128] =  s32; gv[127 - j] = - s32;
    }
  for ( j = 0; j < 16; j++ )
    {
    bu[j] = bu[ 16]; rv[j] = rv[ 16]; gu[j] = gu[ 16]; gv[j] = gv[ 16];
    }
  for ( j = 240; j < 256; j++ )
    {
    bu[j] = bu[239]; rv[j] = rv[239]; gu[j] = gu[239]; gv[j] = gv[239];
    }
  for ( j =  16; j < 236; j++ ) { ay[j] = j; }
  for ( j =   0; j <  16; j++ ) { ay[j] = ay[16]; }
  for ( j = 236; j < 256; j++ ) { ay[j] = ay[235]; }
  JOT(8,"lookup tables are prepared\n");
  }
pcache = peasycap->pcache;
if ((__u8 *)NULL == pcache) pcache = &peasycap->cache[0];
/*-----------------------------------------------------------------------------
TRANSFER CONTENTS OF CACHE TO THE FRAME BUFFER
-----------------------------------------------------------------------------*/
if ( ! pcache ) { SAY("MISTAKE: pcache is NULL\n"); return(-EFAULT); }

if ( pcache != &peasycap->cache[0] )
  {
  JOT(16,"peasycap->cache has %i bytes\n",(int)(pcache - &peasycap->cache[0]));
  }
p2 = &peasycap->cache[0];
p3 = (__u8 *)pad - (int)( pcache - &peasycap->cache[0] );
while ( p2 < pcache ) { *p3++ = *p2;  p2++; }
pcache = &peasycap->cache[0];
if ( p3 != pad ) { SAY("MISTAKE: pointer misalignment\n"); return(-EFAULT); }
/*---------------------------------------------------------------------------*/
rump = (int)(0x03 & mask);
u = 0; v = 0;
...
if ( rump ) JOT(16,"%4i=much  %4i=more  %i=rump\n", much,more,rump);
...
-----------------------------------------------------------------------------*/
        while ( pz > p2 )
          {
          if ( pr <= ( p3 + bytesperpixel ) ) last = true;
          else                                last = false;
          y = *p2;
          ...
          r = ( 255 < s32 ) ? 255 : ((  0 > s32 ) ?  0 : (__u8)s32 );
          s32 = ay[(int)y] - gu[(int)u] - gv[(int)v];
          g = ( 255 < s32 ) ? 255 : ((  0 > s32 ) ?  0 : (__u8)s32 );
          s32 = ay[(int)y] + bu[(int)u];
          b = ( 255 < s32 ) ? 255 : ((  0 > s32 ) ?  0 : (__u8)s32 );
          ...
          if (( true == last ) && rump )
            ...
            switch ( bytesperpixel - rump )
              ...
              case 1 :
                {
                *p3 = r;  *pcache++ = g;  *pcache++ = b;
                break;
                }
              case 2 :
                {
                *p3 = r;  *(p3 + 1) = g;  *pcache++ = b;
                break;
                }
/*
              case 3 :
        ...
        while ( pz > p2 )
          ...
          if (( true == last ) && rump )
            ...
            switch ( bytesperpixel - rump )
              ...
              case 1 :
                {
                *p3 = b;  *pcache++ = g;  *pcache++ = r;
                break;
                }
              case 2 :
                {
                *p3 = b;  *(p3 + 1) = g;  *pcache++ = r;
                break;
                }
/*
              case 3 :
        ...
        while ( pz > p2 )
          ...
          if (( true == last ) && rump )
            ...
            switch ( bytesperpixel - rump )
              ...
              case 1 :
                {
                *p3 = r;  *pcache++ = g;  *pcache++ = b;  *pcache++ = 0;
                break;
                }
              case 2 :
                {
                *p3 = r;  *(p3 + 1) = g;  *pcache++ = b;  *pcache++ = 0;
                break;
                }
              case 3 :
                {
                *p3 = r;  *(p3 + 1) = g;  *(p3 + 2) = b;  *pcache++ = 0;
                break;
                }
              default:
                {
        ...
        while ( pz > p2 )
          ...
          if (( true == last ) && rump )
            ...
            switch ( bytesperpixel - rump )
              ...
              case 1 :
                {
                *p3 = b;  *pcache++ = g;  *pcache++ = r;  *pcache++ = 0;
                break;
                }
              case 2 :
                {
                *p3 = b;  *(p3 + 1) = g;  *pcache++ = r;  *pcache++ = 0;
                break;
                }
              case 3 :
                {
                *p3 = b;  *(p3 + 1) = g;  *(p3 + 2) = r;  *pcache++ = 0;
                break;
                }
              default:
                {
...
if ( NULL != file ) pvma->vm_private_data = file->private_data;
...
if ( NULL != peasycap ) peasycap->vma_many++;
...
if ( ! m ) { JOT(4, "%4i=k, %4i=m\n", k,m); }
else       { JOT(16,"%4i=k, %4i=m\n", k,m); }
...
if ( NULL == ( peasycap = pvma->vm_private_data ))
  {
  SAY("ERROR: peasycap is NULL\n");
  return(retcode);
  }
down( &( peasycap->sem_mmap_video[0] ) );
/*---------------------------------------------------------------------------*/
if ( NULL == ( pbuf = peasycap->frame_buffer[k][m].pgo ))
  {
  SAY("ERROR:  pbuf is NULL\n");
  goto finish;
  }
if ( NULL == ( page = virt_to_page(pbuf) ))
  {
...
THE LOWER BYTE CONTAINS THE FIELD PARITY BYTE FURNISHED BY THE SAA7113H CHIP.
...
if ( peasycap->video_eof ) return;

for ( i = 0; i < VIDEO_ISOC_BUFFER_MANY; i++ )
  {
  if ( purb->transfer_buffer == peasycap->video_isoc_buffer[i].pgo ) break;
  }
JOT(16,"%2i=urb\n", i );
...
if (((( VIDEO_ISOC_BUFFER_MANY - 1 ) == last) && ( 0 != i )) || \
  ...
  {
  JOT(16,"out-of-order urbs %i,%i ... continuing\n",last,i);
  }
...
if ( peasycap->video_idle )
  ...
  if ( peasycap->video_isoc_streaming )
    ...
    if ( 0 != ( rc = usb_submit_urb( purb, GFP_ATOMIC ) ))
      {
      SAY("ERROR: while %i=video_idle, usb_submit_urb() failed with rc:\n", \
                                                       peasycap->video_idle );
      switch (rc)
        {
        case -ENOMEM:   { SAY("ENOMEM\n");    break; }
  ...
  for ( i = 0;  i < purb->number_of_packets; i++ )
    ...
    if ( 0 != purb->iso_frame_desc[i].status )
      ...
      switch ( purb->iso_frame_desc[i].status )
        ...
        default: { strcpy(&errbuf[0],"unknown error" ); break; }
        }
      }
    ...
    if ( ! purb->iso_frame_desc[i].status )
      ...
      if ( 4 < more )
        ...
        pu = (__u8 *)(purb->transfer_buffer + purb->iso_frame_desc[i].offset);
        if ( 0x80 & *pu ) leap = 8;
        else              leap = 4;
/*----------------------------------------------------------------------------
...
THE DECISION ON WHETHER THE PARITY OF THE OFFERED FIELD BUFFER IS RIGHT RESTS
WITH dqbuf().
-----------------------------------------------------------------------------*/
        if (( 8 == more ) || override )
          {
          if ( videofieldamount > peasycap->videofieldamount )
            {
            if ( 2 == videofieldamount - peasycap->videofieldamount ) \
             (peasycap->field_buffer[peasycap->field_fill][0].kount) |= 0x0100;
            else \
             (peasycap->field_buffer[peasycap->field_fill][0].kount) |= 0x4000;
            }
          else if ( videofieldamount < peasycap->videofieldamount )
            {
            (peasycap->field_buffer[peasycap->field_fill][0].kount) |= 0x2000;
            }
          ...
          if ( 0 == ( bad = ( 0xFF00 & \
            ...
            {
            (peasycap->video_junk)--;
            if ( -VIDEO_JUNK_TOLERATE > peasycap->video_junk ) \
                     peasycap->video_junk = -VIDEO_JUNK_TOLERATE;
            ...
            peasycap->video_junk++;
            if ( bad & 0x0010 ) \
                peasycap->video_junk += ( 1 + VIDEO_JUNK_TOLERATE/2 );
            JOT(8,"field buffer %i had %i bytes, now discarded: 0x%04X\n",
                             peasycap->field_fill, videofieldamount,
                 ( 0xFF00 & \
                   peasycap->field_buffer[ peasycap->field_fill ][0].kount ));
          ...
          if ( 8 == more )
            {
            JOT(8,"end-of-field: received parity byte 0x%02X\n",(0xFF & *pu));
            if ( 0x40 & *pu ) pfield_buffer->kount = 0x0000;
            else              pfield_buffer->kount = 0x0001;
            pfield_buffer->input = 0x08 | ( 0x07 & peasycap->input );
...
-----------------------------------------------------------------------------*/
        ...
        while ( more )
          ...
          pfield_buffer = &peasycap->field_buffer\
                          [peasycap->field_fill][peasycap->field_page];
          if ( PAGE_SIZE < ( pfield_buffer->pto - pfield_buffer->pgo ))
            {
          ...
          if ( PAGE_SIZE == ( pfield_buffer->pto - pfield_buffer->pgo ))
            ...
            if ( FIELD_BUFFER_SIZE/PAGE_SIZE <= peasycap->field_page )
              {
              JOT(16,"wrapping peasycap->field_page\n");
              peasycap->field_page = 0;
              }
            pfield_buffer = &peasycap->field_buffer\
                            [peasycap->field_fill][peasycap->field_page];
            pfield_buffer->pto = pfield_buffer->pgo;
            pfield_buffer->input = 0x08 | ( 0x07 & peasycap->input );
            if ( (peasycap->field_buffer[peasycap->field_fill][0]).input != \
                                                        pfield_buffer->input )
                 (peasycap->field_buffer[peasycap->field_fill][0]).kount |= \
                                                                       0x1000;
            }

          much = PAGE_SIZE - (int)( pfield_buffer->pto - pfield_buffer->pgo );
...
THIS CAUSES LOSS OF SYNC, CONFUSING DOWNSTREAM USER-SPACE PROGRAMS WHICH
MAY INTERPRET THE INTERRUPTION AS A SYMPTOM OF LATENCY.  TO OVERCOME THIS
THE DRIVER BRIDGES THE HIATUS BY SENDING DUMMY VIDEO FRAMES AT APPROXIMATELY
THE RIGHT TIME INTERVALS IN THE HOPE OF PERSUADING THE DOWNSTREAM USER-SPACE
...
if ( VIDEO_JUNK_TOLERATE <= peasycap->video_junk )
  {
  JOT(8,"easycap driver shutting down on condition green\n");
  peasycap->status = 1;
  peasycap->video_eof = 1;
  peasycap->video_junk = 0;
  wake_up_interruptible( &(peasycap->wq_video) );
...
struct usb_endpoint_descriptor *pusb_endpoint_descriptor;
struct usb_interface_descriptor *pusb_interface_descriptor;     /* see ch9.h */
struct usb_interface_assoc_descriptor *pusb_interface_assoc_descriptor; /* " */
struct urb *purb;
/* THIS IS INDISPENSIBLY static AND THAT'S A MYSTERY: */
...
void *pbuf;
int viable_altsetting_on[8], is_viable_altsetting_on;
int viable_endpointnumber[8], is_viable_endpointnumber;
int viable_maxpacketsize[8], is_viable_maxpacketsize;
int maxpacketsize;
...
-----------------------------------------------------------------------------*/
if ((struct usb_device *)NULL == \
         ( pusb_device1 = \
             container_of(pusb_interface->dev.parent, struct usb_device, dev)))
  {
...
/*---------------------------------------------------------------------------*/
if ((struct usb_host_interface *)NULL == \
...
/*-----------------------------------------------------------------------------
GET PROPERTIES OF INTERFACE ITSELF
-----------------------------------------------------------------------------*/
...
JOT(4,"intf[%i]: pusb_interface->cur_altsetting - " \
    ...
    (long int)( pusb_interface->cur_altsetting - pusb_interface->altsetting ));
...
/*---------------------------------------------------------------------------*/
if ((struct usb_interface_assoc_descriptor *)NULL != \
...
IT IS NOT POSSIBLE HERE TO FREE ANY EXISTING struct easycap.  THIS SHOULD
HAVE BEEN DONE BY easycap_delete() WHEN THE DEVICE WAS PHYSICALLY UNPLUGGED.
-----------------------------------------------------------------------------*/
if ( 0 == bInterfaceNumber )
  {
  if ( NULL == ( peasycap = kzalloc(sizeof(struct easycap), GFP_KERNEL) ))
    {
...
-----------------------------------------------------------------------------*/
  ...
  init_MUTEX( &( peasycap->sem_timeval0 ) );
  init_MUTEX( &( peasycap->sem_timeval1 ) );
  for ( k = 0; k < FRAME_BUFFER_MANY; k++ )
    {
    init_MUTEX( &( peasycap->sem_mmap_video[k] ) );
    }
  ...
  if ((struct semaphore *)NULL == &( peasycap->sem_mmap_video[0] ))
    {
    SAY("ERROR: &(peasycap->sem_mmap_video[%i]) is NULL.\n", 0 );
    return(-EFAULT);
    }
/*-----------------------------------------------------------------------------
DYNAMICALLY FILL IN THE AVAILABLE FORMATS ...
-----------------------------------------------------------------------------*/
  if ( 0 > ( rc = fillin_formats() ))
    {
...
-----------------------------------------------------------------------------*/
  ...
  while ( 0xFFFF != easycap_standard[i].mask )
    ...
    if ( NTSC_M == easycap_standard[i].v4l2_standard.index )
      {
      m++;
      for ( k = 0; k < 6; k++ ) { peasycap->inputset[k].standard_offset = i; }
      mask = easycap_standard[i].mask;
      }
#else
    if ( PAL_BGHIN == easycap_standard[i].v4l2_standard.index )
      {
      m++;
      for ( k = 0; k < 6; k++ ) { peasycap->inputset[k].standard_offset = i; }
      mask = easycap_standard[i].mask;
  ...
  if ( 1 != m )
    {
    SAY("MISTAKE: easycap.inputset[].standard_offset unpopulated\n");
    return(-ENOENT);
    }

  peasycap_format = &easycap_format[0];  i = 0;  m = 0;
  while ( 0 != peasycap_format->v4l2_format.fmt.pix.width )
    ...
    if ((( peasycap_format->mask & 0x0F ) == ( mask & 0x0F )) && \
         ...
         ( peasycap_format->v4l2_format.fmt.pix.pixelformat == \
                                                    V4L2_PIX_FMT_UYVY ) && \
         ( peasycap_format->v4l2_format.fmt.pix.width  == 640 ) && \
       (   peasycap_format->v4l2_format.fmt.pix.height == 480 ))
      {
      m++;
      for ( k = 0; k < 6; k++ ) { peasycap->inputset[k].format_offset = i; }
      break;
      }
    peasycap_format++;  i++;
    }
  if ( 1 != m )
    {
    SAY("MISTAKE: easycap.inputset[].format_offset unpopulated\n");
    return(-ENOENT);
    }
  ...
  while ( 0xFFFFFFFF != easycap_control[i].id )
    ...
    if ( V4L2_CID_BRIGHTNESS == easycap_control[i].id )
      {
      m++;
      for ( k = 0; k < 6; k++ ) { peasycap->inputset[k].brightness = value; }
      }
    else if ( V4L2_CID_CONTRAST == easycap_control[i].id )
      {
      m++;
      for ( k = 0; k < 6; k++ ) { peasycap->inputset[k].contrast = value; }
      }
    else if ( V4L2_CID_SATURATION == easycap_control[i].id )
      {
      m++;
      for ( k = 0; k < 6; k++ ) { peasycap->inputset[k].saturation = value; }
      }
    else if ( V4L2_CID_HUE == easycap_control[i].id )
      {
      m++;
      for ( k = 0; k < 6; k++ ) { peasycap->inputset[k].hue = value; }
      }
    i++;
    }
  if ( 4 != m )
    {
    SAY("MISTAKE: easycap.inputset[].brightness,... underpopulated\n");
    return(-ENOENT);
    }
  for ( k = 0; k < 6; k++ ) { peasycap->inputset[k].input = k; }
  JOT(4,"populated easycap.inputset[]\n");
  ...
  if ((struct easycap *)NULL == peasycap )
    {
    SAY("ERROR: " \
          "peasycap is NULL when probing interface %i\n", bInterfaceNumber );
    return(-EFAULT);
    }
...
if (( USB_CLASS_VIDEO == bInterfaceClass ) || \
  ...
  if ( -1 == peasycap->video_interface )
    ...
    if ( peasycap->video_interface != bInterfaceNumber )
      {
      SAY("ERROR: attempting to reset peasycap->video_interface\n");
  ...
  if ( -1 == peasycap->audio_interface )
    ...
    if ( peasycap->audio_interface != bInterfaceNumber )
      {
      SAY("ERROR: attempting to reset peasycap->audio_interface\n");
...
-----------------------------------------------------------------------------*/
is_viable_altsetting_on = 0;
is_viable_endpointnumber = 0;
is_viable_maxpacketsize = 0;
...
/*---------------------------------------------------------------------------*/
  for ( j = 0; j < pusb_interface_descriptor->bNumEndpoints; j++ )
    {
    if ((struct usb_endpoint_descriptor *)NULL == \
             ( pusb_endpoint_descriptor = \
                   &(pusb_host_interface->endpoint[j].desc)) )
      {
      SAY("ERROR:  pusb_endpoint_descriptor is NULL.\n");
      SAY("...... skipping\n");
      }
    else
      {
      wMaxPacketSize = le16_to_cpu(pusb_endpoint_descriptor->wMaxPacketSize);
      bEndpointAddress = pusb_endpoint_descriptor->bEndpointAddress;

      JOT(4,"intf[%i]alt[%i]end[%i]: bEndpointAddress=0x%X\n", \
         bInterfaceNumber, i, j, pusb_endpoint_descriptor->bEndpointAddress);
      JOT(4,"intf[%i]alt[%i]end[%i]: bmAttributes=0x%X\n", \
         bInterfaceNumber, i, j, pusb_endpoint_descriptor->bmAttributes);
      JOT(4,"intf[%i]alt[%i]end[%i]: wMaxPacketSize=%i\n", \
         bInterfaceNumber, i, j, pusb_endpoint_descriptor->wMaxPacketSize);
      JOT(4,"intf[%i]alt[%i]end[%i]: bInterval=%i\n",
         bInterfaceNumber, i, j, pusb_endpoint_descriptor->bInterval);

      if (pusb_endpoint_descriptor->bEndpointAddress & USB_DIR_IN)
        {
      ...
      if ((pusb_endpoint_descriptor->bmAttributes & \
        ...
        if ( isin )
          ...
          switch ( bInterfaceClass )
            ...
            case USB_CLASS_VENDOR_SPEC :
              ...
              if ( pusb_endpoint_descriptor->wMaxPacketSize )
                {
                if ( 8 > is_viable_altsetting_on )
                  {
                  viable_altsetting_on[ is_viable_altsetting_on ] = i ;
                  JOT(4,"%i=viable_altsetting_on\n", \
                              viable_altsetting_on[ is_viable_altsetting_on ]);
                  is_viable_altsetting_on++;
                  }
                if ( 8 > is_viable_endpointnumber )
                  {
                  viable_endpointnumber[ is_viable_endpointnumber ] = \
                             pusb_endpoint_descriptor->bEndpointAddress & 0x0F;
                  JOT(4,"%i=viable_endpointnumber\n", \
                            viable_endpointnumber[ is_viable_endpointnumber ]);
                  is_viable_endpointnumber++;
                  }
                if ( 8 > is_viable_maxpacketsize )
                  {
                  viable_maxpacketsize[ is_viable_maxpacketsize ] = \
                         le16_to_cpu(pusb_endpoint_descriptor->wMaxPacketSize);
                  JOT(4,"%i=viable_maxpacketsize\n", \
                              viable_maxpacketsize[ is_viable_maxpacketsize ]);
                  is_viable_maxpacketsize++;
                  }
                }
              else
                {
                if ( -1 == peasycap->video_altsetting_off )
                  ...
                  JOT(4,"%i=video_altsetting_off <====\n", \
                                             peasycap->video_altsetting_off);
                  }
                else
                  {
                  SAY("ERROR: peasycap->video_altsetting_off already set\n");
                  SAY("...... continuing with " \
                      "%i=peasycap->video_altsetting_off\n", 
                          peasycap->video_altsetting_off);
                  }
                }
              break;
              ...
              if ( 0x02 == bInterfaceSubClass )
                ...
                if ( pusb_endpoint_descriptor->wMaxPacketSize )
                  ...
                  if ( -1 == peasycap->audio_altsetting_off )
                    ...
                    JOT(4,"%i=peasycap->audio_altsetting_off <====\n", \
                                               peasycap->audio_altsetting_off);
                    }
                  else
                    {
                    SAY("ERROR: peasycap->audio_altsetting_off already set\n");
                    SAY("...... continuing with " \
                        "%i=peasycap->audio_altsetting_off\n", 
                            peasycap->audio_altsetting_off);
                    }
                  }
                }
      ...
      else if ((pusb_endpoint_descriptor->bmAttributes & \
      ...
      if ( 0 == pusb_endpoint_descriptor->wMaxPacketSize )
        {
        JOT(4,"intf[%i]alt[%i]end[%i] has zero packet size\n", \
                                        bInterfaceNumber, i, j );
        }
      }
    }
...
-----------------------------------------------------------------------------*/
  case 0 :
    ...
    if ( ! is_viable_altsetting_on )
      ...
      {
      peasycap->video_altsetting_on = \
               viable_altsetting_on[ is_viable_altsetting_on - 1 ];
      JOT(4,"%i=video_altsetting_on <====\n", peasycap->video_altsetting_on);
      }
    if ( ! is_viable_endpointnumber )
      {
      ...
      {
      peasycap->video_endpointnumber = \
               viable_endpointnumber[ is_viable_endpointnumber - 1 ];
      JOT(4,"%i=video_endpointnumber\n", peasycap->video_endpointnumber);
      }
    if ( ! is_viable_maxpacketsize )
      {
...
-----------------------------------------------------------------------------*/
      ...
      {
      maxpacketsize = viable_maxpacketsize[ is_viable_maxpacketsize - 1 ] \
                               -1024;
      ...
      peasycap->video_isoc_buffer_size = peasycap->video_isoc_maxframesize *
                                         peasycap->video_isoc_framesperdesc;
      JOT(4,"%i=video_isoc_buffer_size\n", peasycap->video_isoc_buffer_size);
      if (( PAGE_SIZE << VIDEO_ISOC_ORDER ) < peasycap->video_isoc_buffer_size)
        {
        SAY("MISTAKE: peasycap->video_isoc_buffer_size too big\n");
        return(-EFAULT);
        }
      }
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
    ...
    for ( k = 0;  k < FRAME_BUFFER_MANY;  k++ )
      ...
      for ( m = 0;  m < FRAME_BUFFER_SIZE/PAGE_SIZE;  m++ )
        ...
        if ((void *)NULL != peasycap->frame_buffer[k][m].pgo )
          {
          SAY("attempting to reallocate frame buffers\n");
          }
        else
          {
          if ((void *)NULL == ( pbuf = (void *)__get_free_page(GFP_KERNEL)))
            {
            SAY("ERROR: Could not allocate frame buffer %i page %i\n", k,m);
            return(-ENOMEM);
            }
          else peasycap->allocation_video_page += 1;
...
/*---------------------------------------------------------------------------*/
    ...
    for ( k = 0;  k < FIELD_BUFFER_MANY;  k++ )
      ...
      for ( m = 0;  m < FIELD_BUFFER_SIZE/PAGE_SIZE;  m++ )
        ...
        if ((void *)NULL != peasycap->field_buffer[k][m].pgo )
          {
          SAY("ERROR: attempting to reallocate field buffers\n");
          }
        else
          {
          if ((void *)NULL == ( pbuf = (void *)__get_free_page(GFP_KERNEL)))
            {
            SAY("ERROR: Could not allocate field buffer %i page %i\n", k,m);
            return(-ENOMEM);
            }
          else peasycap->allocation_video_page += 1;
...
/*---------------------------------------------------------------------------*/
    JOT(4,"allocating %i isoc video buffers of size %i\n",  \
                     VIDEO_ISOC_BUFFER_MANY, peasycap->video_isoc_buffer_size);
    JOT(4,".... each occupying contiguous memory pages\n");

    for ( k = 0;  k < VIDEO_ISOC_BUFFER_MANY; k++ )
      {
      if (NULL == ( pbuf = (void *)__get_free_pages(GFP_KERNEL, \
                                               VIDEO_ISOC_ORDER)))
        {
        SAY("ERROR: Could not allocate isoc video buffer %i\n", k);
        return(-ENOMEM);
        }
      else peasycap->allocation_video_page += \
                                    ((unsigned int)(0x01 << VIDEO_ISOC_ORDER));
...
-----------------------------------------------------------------------------*/
    ...
    for ( k = 0;  k < VIDEO_ISOC_BUFFER_MANY; k++ )
      {
      if ( NULL == ( purb = usb_alloc_urb(peasycap->video_isoc_framesperdesc, \
                                                                 GFP_KERNEL) ))
        {
        SAY("ERROR: usb_alloc_urb returned NULL for buffer %i\n", k);
        return(-ENOMEM);
        }
      else peasycap->allocation_video_urb += 1       ;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
      if ( NULL == ( pdata_urb = \
                kzalloc(sizeof(struct data_urb), GFP_KERNEL) ))
        {
        SAY("ERROR: Could not allocate struct data_urb.\n");
        return(-ENOMEM);
        }
      else peasycap->allocation_video_struct += sizeof(struct data_urb);
...
-----------------------------------------------------------------------------*/
      if ( ! k )
        ...
        JOT(4,"  purb->transfer_buffer = " \
                             "peasycap->video_isoc_buffer[.].pgo;\n");
      ...
      for ( j = 0;  j < peasycap->video_isoc_framesperdesc; j++ )
        {
        purb->iso_frame_desc[j].offset = j * peasycap->video_isoc_maxframesize;
        purb->iso_frame_desc[j].length = peasycap->video_isoc_maxframesize;
        }
      }
...
-----------------------------------------------------------------------------*/
    ...
    else ( peasycap->registered_video )++;
    SAY("easycap attached to minor #%d\n", pusb_interface->minor);
    break;
/*vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv*/
#else
    if ((struct video_device *)NULL == ( pvideo_device = \
              (struct video_device *) kzalloc(sizeof(struct video_device), \
                                                               GFP_KERNEL) ))
      {
    ...
    if ( 0 != ( video_register_device(pvideo_device, VFL_TYPE_GRABBER, -1 )))
      {
  ...
  case 2:
    ...
    if ( ! is_viable_maxpacketsize )
      ...
      peasycap->audio_isoc_maxframesize = \
                viable_maxpacketsize[ is_viable_maxpacketsize - 1 ];
      ...
      if (9 == peasycap->audio_isoc_maxframesize)
        {
        peasycap->ilk |= 0x02;  SAY("hardware is FOUR-CVBS\n");
        peasycap->microphone = true;
        peasycap->audio_pages_per_fragment = 4;
        }
      else if (256 == peasycap->audio_isoc_maxframesize)
        {
        peasycap->ilk &= ~0x02;  SAY("hardware is CVBS+S-VIDEO\n");
        peasycap->microphone = false;
        peasycap->audio_pages_per_fragment = 4;
        }
      else
      ...
      peasycap->audio_isoc_buffer_size = peasycap->audio_isoc_maxframesize *
                                         peasycap->audio_isoc_framesperdesc;
...
-----------------------------------------------------------------------------*/
    ...
    for ( k = 0;  k < peasycap->audio_buffer_page_many;  k++ )
      ...
      if ((void *)NULL != peasycap->audio_buffer[k].pgo )
        ...
        if ((void *)NULL == ( pbuf = (void *)__get_free_page(GFP_KERNEL)))
          {
          SAY("ERROR: Could not allocate audio buffer page %i\n", k);
          return(-ENOMEM);
          }
        else peasycap->allocation_audio_page += 1;
    ...
    for ( k = 0;  k < AUDIO_ISOC_BUFFER_MANY; k++ )
      ...
      if ( 1 )
        ...
        if (NULL == ( pbuf = (void *)__get_free_pages(GFP_KERNEL, \
                                                 AUDIO_ISOC_ORDER)))
          {
          SAY("ERROR: Could not allocate isoc audio buffer %i\n", k);
          return(-ENOMEM);
          }
        else peasycap->allocation_audio_page += \
                                    ((unsigned int)(0x01 << AUDIO_ISOC_ORDER));
        ...
        {
        SAY("ERROR: already allocated isoc audio buffer %i.\n", k);
        SAY("...... continuing\n");
        }
      }
...
-----------------------------------------------------------------------------*/
    ...
    for ( k = 0;  k < AUDIO_ISOC_BUFFER_MANY; k++ )
      {
      if ( NULL == ( purb = usb_alloc_urb(peasycap->audio_isoc_framesperdesc, \
                                                                 GFP_KERNEL) ))
        {
        SAY("ERROR: usb_alloc_urb returned NULL for buffer %i\n", k);
        return(-ENOMEM);
        }
      else peasycap->allocation_audio_urb += 1 ;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
      if ( NULL == ( pdata_urb = \
                kzalloc(sizeof(struct data_urb), GFP_KERNEL) ))
        {
      ...
      else peasycap->allocation_audio_struct += sizeof(struct data_urb);
...
-----------------------------------------------------------------------------*/
    if ( 0 != ( usb_register_dev(pusb_interface, &easysnd_class )))
      {
      err("Not able to get a minor for this device.");
      usb_set_intfdata(pusb_interface, NULL);
      return(-ENODEV);
      }
    else ( peasycap->registered_audio )++;
/*-----------------------------------------------------------------------------
...
/*****************************************************************************/
/*-----------------------------------------------------------------------------
WHEN THIS FUNCTION IS CALLED THE DEVICE HAS ALREADY BEEN PHYSICALLY UNPLUGGED.
...
if ((struct easycap *)NULL == ( peasycap = usb_get_intfdata(pusb_interface) ))
  {
  SAY("ERROR: peasycap is NULL\n");
  }
/*---------------------------------------------------------------------------*/
...
if ( 0 != ( result = usb_register(&easycap_usb_driver) )) \
