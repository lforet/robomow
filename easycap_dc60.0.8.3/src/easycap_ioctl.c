...
/*----------------------------------------------------------------------------
UNLESS THERE IS A PREMATURE ERROR RETURN THIS ROUTINE UPDATES THE FOLLOWING:
...
while ( 0xFFFF != peasycap_standard->mask )
  {
  if ( std_id & peasycap_standard->v4l2_standard.id ) break;
  peasycap_standard++;
  }
...
switch ( peasycap_standard->mask & 0x000F )
  ...
  case NTSC_M_JP :
    {
    reg = 0x0A;  set = 0x95;
    if ( 0 > ( ir = read_saa(peasycap->pusb_device, reg )))
      {
      SAY("ERROR: cannot read SAA register 0x%02X\n", reg );
      }
    else itwas = (unsigned int)ir;
    if ( 0 != write_saa(peasycap->pusb_device, reg, set ))
      ...
      if ( 0 > ir )
        ...
        {
        JOT(8,"SAA register 0x%02X changed from 0x%02X to 0x%02X\n", \
                                                      reg,itwas,isnow);
        }
      }
...
----------------------------------------------------------------------------*/
  ...
  case NTSC_M :
  case PAL_BGHIN : { reg = 0x0E;  set = 0x01;  need = 1;  break; }
  case NTSC_N_443 :
  case PAL_60 :    { reg = 0x0E;  set = 0x11;  need = 1;  break; }
  case NTSC_443 :
  case PAL_Nc :    { reg = 0x0E;  set = 0x21;  need = 1;  break; }
  case NTSC_N :
  case PAL_M :     { reg = 0x0E;  set = 0x31;  need = 1;  break; }
  case SECAM :     { reg = 0x0E;  set = 0x51;  need = 1;  break; }
  default:         { break; }
  }
/*--------------------------------------------------------------------------*/
if ( need )
...
if ( 0 > ( ir = read_saa(peasycap->pusb_device, reg ))) \
  {
  SAY("ERROR: failed to read SAA register 0x%02X so cannot reset\n",reg);
  }
else
  {
  itwas = (unsigned int)ir;
  if ( peasycap_standard->mask & 0x0001 ) set = itwas | 0x40 ;
  else                                    set = itwas & ~0x40 ;
  if ( 0 != write_saa(peasycap->pusb_device, reg, set ))
    {
    SAY("ERROR: failed to set SAA register 0x%02X to 0x%02X\n",reg,set);
...
if ( 0 > ( ir = read_saa(peasycap->pusb_device, reg ))) \
  ...
  {
  itwas = (unsigned int)ir;
  if ( peasycap_standard->mask & 0x0001 ) set = itwas | 0x80 ;
  else                                    set = itwas & ~0x80 ;
  if ( 0 != write_saa(peasycap->pusb_device, reg, set ))
    {
    SAY("ERROR: failed to set SAA register 0x%02X to 0x%02X\n",reg,set);
...
itwas = (unsigned int)ir;
if ( peasycap_standard->mask & 0x0001 ) set = 0x0A ;
else                                    set = 0x07 ;
if ( 0 != write_saa(peasycap->pusb_device, reg, set ))
  {
  SAY("ERROR: failed to set SAA register 0x%02X to 0x%02X\n",reg,set);
...
IF SUCCESSFUL THE FUNCTION RETURNS THE OFFSET IN easycap_format[] IDENTIFYING
THE FORMAT WHICH IS TO RETURNED TO THE USER.
...
if ((struct usb_device *)NULL == ( p = peasycap->pusb_device ))
  {
...
if ( false == try )
  ...
  if ( true == peasycap->offerfields )
    {
    SAY("WARNING: %i=peasycap->field is untested: please report problems\n", \
                                                             peasycap->field );

/*---- UNTESTED, MAY BE (AND PROBABLY IS) INCORRECT:
...
-----------------------------------------------------------------------------*/
  if ( 0 == ( 0x01 & peasycap_format->mask ) )
    ...
    if ((( 720 == peasycap_format->v4l2_format.fmt.pix.width ) && \
      ...
      if ( 0 != set_resolution(p,0x0008,0x0020,0x0508,0x0110) )
#else
...
-----------------------------------------------------------------------------*/
    ...
    if ((( 720 == peasycap_format->v4l2_format.fmt.pix.width ) && \
      ...
      {
#if defined(OLDMARGIN)
      if ( 0 != set_resolution(p,0x0008,0x0003,0x0508,0x00F3) )
#else
...
while ( 0xFFFFFFFF != easycap_control[i1].id )
  ...
  if ( V4L2_CID_BRIGHTNESS == easycap_control[i1].id )
    ...
    if ( ! write_saa(peasycap->pusb_device,0x0A,mood))
      ...
      {
      SAY("WARNING: failed to adjust brightness to 0x%02X\n", mood );
      return(-ENOENT);
      }
    break;
    }
...
while ( 0xFFFFFFFF != easycap_control[i1].id )
  ...
  if ( V4L2_CID_CONTRAST == easycap_control[i1].id )
    ...
    if ( ! write_saa(peasycap->pusb_device,0x0B,mood))
      ...
      {
      SAY("WARNING: failed to adjust contrast to 0x%02X\n", mood );
      return(-ENOENT);
      }
    break;
    }
...
while ( 0xFFFFFFFF != easycap_control[i1].id )
  ...
  if ( V4L2_CID_SATURATION == easycap_control[i1].id )
    ...
    if ( ! write_saa(peasycap->pusb_device,0x0C,mood))
      ...
      {
      SAY("WARNING: failed to adjust saturation to 0x%02X\n", mood );
      return(-ENOENT);
      }
    break;
    }
...
while ( 0xFFFFFFFF != easycap_control[i1].id )
  ...
  if ( V4L2_CID_AUDIO_VOLUME == easycap_control[i1].id )
    ...
    mood = ( 16 > peasycap->volume ) ? 16 : \
          (( 31 < peasycap->volume ) ? 31 : (__s8) peasycap->volume );
    if ( ! audio_gainset( peasycap->pusb_device,mood ))
      ...
      {
      SAY("WARNING: failed to adjust volume to 0x%1X\n", mood );
      return(-ENOENT);
      }
    break;
    }
...
easycap_ioctl.c: warning: the frame size of ... bytes is larger than 1024 bytes
-----------------------------------------------------------------------------*/
switch (cmd)
  {
  case VIDIOC_QUERYCAP:
    ...
    while ( *p2 )
      {
      p1 = p2;  while ( *p2 && ( '.' != *p2 )) { p2++; }
      if ( *p2 ) *p2++ = 0;
      if ( 3 > i ) k[i] = (int) simple_strtol( p1, NULL, 10 );
      i++;
      }
    ...
    if ( 0 != copy_to_user((void __user *)arg, &v4l2_capability, \
                      sizeof(struct v4l2_capability))) {POUT;return(-EFAULT);}
    break;
    }
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_ENUMINPUT:
    ...
    if ( 0 != copy_from_user(&v4l2_input, (void __user *)arg, \
                        sizeof(struct v4l2_input))) {POUT;return(-EFAULT);}
    ...
    switch ( index )
      ...
      case 0 :
        ...
        v4l2_input.tuner = 0;
        v4l2_input.std = V4L2_STD_PAL | V4L2_STD_SECAM | V4L2_STD_NTSC ;
        v4l2_input.status = 0;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_G_INPUT:
    ...
    if ( 0 != copy_to_user((void __user *)arg, &index, \
                                  sizeof(__u32))) {POUT;return(-EFAULT);}
    break;
    }
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_S_INPUT:
    ...
    if ( 0 != copy_from_user(&index, (void __user *)arg, \
                                  sizeof(__u32))) {POUT;return(-EFAULT);}
    SAY("user requests input %i\n", index);
    ...
    if (( 0 > index ) || ( 5 < index ))
      {
      SAY("ERROR:  bad requested input: %i\n",index);
      return(-EINVAL);
      }
    rc = newinput(peasycap, (int)index);
    if ( 0 == rc ) { JOT(8,"newinput(.,%i) OK\n",(int)index); }
    else
      {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_ENUMAUDOUT:
    ...
    if ( 0 != copy_from_user(&v4l2_audioout, (void __user *)arg, \
                      sizeof(struct v4l2_audioout))) {POUT;return(-EFAULT);}

    if ( 0 != v4l2_audioout.index ) return(-EINVAL);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_QUERYCTRL:
    ...
    if ( 0 != copy_from_user(&v4l2_queryctrl, (void __user *)arg, \
                      sizeof(struct v4l2_queryctrl))) {POUT;return(-EFAULT);}
    ...
    while ( 0xFFFFFFFF != easycap_control[i1].id )
      ...
      if (easycap_control[i1].id == v4l2_queryctrl.id)
        {
        JOT(8,"VIDIOC_QUERYCTRL  %s=easycap_control[%i].name\n", \
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_G_CTRL:
    ...
    if ( 0 != copy_from_user(&v4l2_control, (void __user *)arg, \
                      sizeof(struct v4l2_control))) {POUT;return(-EFAULT);}
    ...
    switch (v4l2_control.id)
      ...
      case V4L2_CID_AUDIO_MUTE:
        {
        if ( 1 == peasycap->mute ) v4l2_control.value = true;
        else                       v4l2_control.value = false;
  ...
  case VIDIOC_S_CTRL:
    ...
    switch (v4l2_control.id)
      ...
      case V4L2_CID_AUDIO_MUTE:
        ...
        if ( true == v4l2_control.value ) mute = 1;
        else                              mute = 0;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_ENUM_FMT:
    ...
    if ( 0 != copy_from_user(&v4l2_fmtdesc, (void __user *)arg, \
                      sizeof(struct v4l2_fmtdesc))) {POUT;return(-EFAULT);}
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_ENUM_FRAMESIZES:
    {
    static __u32 index;
    static struct v4l2_frmsizeenum v4l2_frmsizeenum;

    JOT(8,"VIDIOC_ENUM_FRAMESIZES\n");

    if ( 0 != copy_from_user(&v4l2_frmsizeenum, (void __user *)arg, \
                      sizeof(struct v4l2_frmsizeenum))) {POUT;return(-EFAULT);}
    ...
    switch (index)
      ...
      case 0:
        ...
        JOT(8,"%i=index: %ix%i\n",index, \
                                  (int)(v4l2_frmsizeenum.discrete.width), \
                                  (int)(v4l2_frmsizeenum.discrete.height));
        break;
        }
      case 1:
        {
  ...
  case VIDIOC_ENUM_FRAMEINTERVALS:
    {
    static __u32 index;
    static struct v4l2_frmivalenum v4l2_frmivalenum;

    JOT(8,"VIDIOC_ENUM_FRAMEINTERVALS\n");
    ...
    if ( 0 != copy_from_user(&v4l2_frmivalenum, (void __user *)arg, \
                      sizeof(struct v4l2_frmivalenum))) {POUT;return(-EFAULT);}
    ...
    switch (index)
      ...
      case 0:
        ...
        v4l2_frmivalenum.discrete.denominator = peasycap->fps;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_G_FMT:
    ...
    if ( 0 != copy_from_user(&v4l2_format, (void __user *)arg, \
                      sizeof(struct v4l2_format))) {POUT;return(-EFAULT);}

    if (v4l2_format.type != V4L2_BUF_TYPE_VIDEO_CAPTURE){POUT;return(-EINVAL);}

    memset(&v4l2_pix_format, 0, sizeof(struct v4l2_pix_format));
    v4l2_format.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    memcpy(&(v4l2_format.fmt.pix), \
       &(easycap_format[ peasycap->format_offset ].v4l2_format.fmt.pix), \
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  case VIDIOC_S_FMT:
    ...
    if ( 0 > ( best_format = adjust_format(peasycap, \
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_CROPCAP:
    ...
    if ( 0 != copy_from_user(&v4l2_cropcap, (void __user *)arg, \
                      sizeof(struct v4l2_cropcap))) {POUT;return(-EFAULT);}
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_QUERYSTD:
    {
    JOT(8,"VIDIOC_QUERYSTD: EasyCAP is incapable of detecting standard\n");
    return(-EINVAL);
    break;
    }
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
-----------------------------------------------------------------------------*/
  case VIDIOC_ENUMSTD:
    ...
    if ( 0 != copy_from_user(&v4l2_standard, (void __user *)arg, \
                  sizeof(struct v4l2_standard))) {POUT;return(-EFAULT);}
    ...
    while ( 0xFFFF != peasycap_standard->mask )
      {
      if ((int)( peasycap_standard - &easycap_standard[0] ) == index ) break;
      peasycap_standard++;
      }
    if ( 0xFFFF == peasycap_standard->mask ) 
      {
    ...
    JOT(8,"%i=index: %s\n",index,&(peasycap_standard->v4l2_standard.name[0]));
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_G_STD:
    ...
    if ( 0 != copy_from_user(&std_id, (void __user *)arg, \
                  sizeof(v4l2_std_id))) {POUT;return(-EFAULT);}
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_S_STD:
    ...
    if ( 0 > ( rc = adjust_standard( peasycap, std_id )) )
      {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_REQBUFS:
    ...
    if ( 0 != copy_from_user(&v4l2_requestbuffers, (void __user *)arg, \
                  sizeof(struct v4l2_requestbuffers))) {POUT;return(-EFAULT);}

    if (v4l2_requestbuffers.type != V4L2_BUF_TYPE_VIDEO_CAPTURE) \
                                                   return(-EINVAL);
    if (v4l2_requestbuffers.memory != V4L2_MEMORY_MMAP){POUT; return(-EINVAL);}
    nbuffers = v4l2_requestbuffers.count;
    JOT(8,"                   User requests %i buffers ...\n", nbuffers);
    if (nbuffers < 2) nbuffers = 2;
    if (nbuffers > FRAME_BUFFER_MANY) nbuffers = FRAME_BUFFER_MANY;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_QUERYBUF:
    ...
    if ( 0 != copy_from_user(&v4l2_buffer, (void __user *)arg, \
                  sizeof(struct v4l2_buffer))) {POUT;return(-EFAULT);}

    if (v4l2_buffer.type != V4L2_BUF_TYPE_VIDEO_CAPTURE) return(-EINVAL);
    index = v4l2_buffer.index;
    if (index < 0 || index >= peasycap->frame_buffer_many ) return(-EINVAL);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_QBUF:
    ...
    if (v4l2_buffer.type != V4L2_BUF_TYPE_VIDEO_CAPTURE) return(-EINVAL);
    if (v4l2_buffer.memory != V4L2_MEMORY_MMAP) return(-EINVAL);
    if ( v4l2_buffer.index < 0 || \
       ( v4l2_buffer.index >= peasycap->frame_buffer_many )) return(-EINVAL);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_DQBUF:
    ...
    static struct timeval timeval8;
    ...
    if (( peasycap->video_idle ) || ( peasycap->video_eof ))
      {
      JOT(8,"returning -EIO because  %i=video_idle  %i=video_eof\n", \
...
/*-----------------------------------------------------------------------------
IF THE USER HAS PREVIOUSLY CALLED easycap_poll(), AS DETERMINED BY FINDING THE
FLAG peasycap->polled SET, THERE MUST BE NO FURTHER WAIT HERE.  IN THIS CASE,
JUST CHOOSE THE FRAME INDICATED BY peasycap->frame_read
-----------------------------------------------------------------------------*/

    if ( ! peasycap->polled )
      {
      while ( 0 != ( rcdq = easycap_dqbuf(peasycap,0) ))
        {
        if ( -EIO == easycap_dqbuf(peasycap,0) )
          {
          JOT(8,"returning -EIO because dqbuf() returned -EIO\n" );
          return( -EIO );
          }
        }
      }
      ...
      if ( peasycap->video_eof )
        {
        JOT(8,"returning -EIO because %i=peasycap->video_eof\n", \
                                             peasycap->video_eof );
        return( -EIO );
        }
      }
    ...
    if ( ! ( peasycap->isequence % 10 ) )
      ...
      j = 0; for ( i = 0; i < 180; i++ ) { j += peasycap->merit[i]; }
      if ( 90 < j )
        {
        SAY("easycap driver shutting down on condition blue\n");
    ...
    if ( V4L2_FIELD_ALTERNATE == v4l2_buffer.field )
      {
      v4l2_buffer.field = \
            0x000F & (peasycap->frame_buffer[ peasycap->frame_read ][0].kount);
      }
    ...
    if ( ! peasycap->timeval0.tv_sec )
      ...
      timeval2 = timeval;
      dnbydt = 192000;
      if (down_interruptible(&( peasycap->sem_timeval0))) return(-ERESTARTSYS);
      peasycap->timeval0 = timeval8;
      up( &( peasycap->sem_timeval0 ));
      }
    else
      {
      if (down_interruptible(&( peasycap->sem_timeval1))) return(-ERESTARTSYS);
      dnbydt = peasycap->dnbydt;
      timeval1 = peasycap->timeval1;
      up( &( peasycap->sem_timeval1 ));
    ...
    if ( ! ( peasycap->isequence % 500) )
      ...
      fudge = ((long long int)( 1000000 )) * \
              ((long long int)( timeval.tv_sec  - timeval2.tv_sec )) + \
               (long long int)( timeval.tv_usec - timeval2.tv_usec );
    ...
    JOT(16, "  0x%08X=flags\n",v4l2_buffer.flags);
    JOT(16, "  %10i=field\n",v4l2_buffer.field);
    JOT( 8, "  %10li=timestamp.tv_sec\n", \
             (long)v4l2_buffer.timestamp.tv_sec);
    JOT( 8, "  %10li=timestamp.tv_usec\n", \
             (long)v4l2_buffer.timestamp.tv_usec);
    JOT( 8, "  %10i=sequence\n",v4l2_buffer.sequence);
    JOT(16, "  0x%08X=memory\n",v4l2_buffer.memory);
    JOT(16, "  %10i=m.offset\n",v4l2_buffer.m.offset);
    ...
    JOT(8,"user is offered frame buffer %i, input 0x%02X\n", \
                                     peasycap->frame_read, \
              peasycap->frame_buffer[peasycap->frame_read][0].input );
    ...
    if ( peasycap->frame_read == peasycap->frame_fill )
      ...
      if ( peasycap->frame_lock )
        {
        JOT(8,"ERROR:  filling frame buffer while offered to user.\n");
        }
      }
    break;
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  case VIDIOC_G_PARM:
    ...
    if ( 0 != copy_from_user(&v4l2_streamparm, (void __user *)arg, \
                  sizeof(struct v4l2_streamparm))) {POUT;return(-EFAULT);}
...
switch (cmd)
...
#if defined(UPSAMPLE)
    if ( true == peasycap->microphone ) caps = 0x04400000;
    else                                caps = 0x04400000;
#else
    if ( true == peasycap->microphone ) caps = 0x02400000;
    else                                caps = 0x04400000;
#endif /*UPSAMPLE*/

    if ( 0 != copy_to_user((void __user *)arg, &caps, \
...
#if defined(UPSAMPLE)
    if ( true == peasycap->microphone) incoming = AFMT_S16_LE ;
    else                               incoming = AFMT_S16_LE ;
#else
    if ( true == peasycap->microphone) incoming = AFMT_S16_LE ;
    else                               incoming = AFMT_S16_LE ;
#endif /*UPSAMPLE*/

    if ( 0 != copy_to_user((void __user *)arg, &incoming, \
                  sizeof(int))) return(-EFAULT);
    break;
    }
  case SNDCTL_DSP_SETFMT:
    ...
    if ( 0 != copy_from_user(&incoming, (void __user *)arg, \
                                sizeof(int))) return(-EFAULT);
    JOT(8,"........... %i=incoming\n", incoming );

#if defined(UPSAMPLE)
    if ( true == peasycap->microphone ) outgoing = AFMT_S16_LE ;
    else                                outgoing = AFMT_S16_LE ;
#else
    if ( true == peasycap->microphone ) outgoing = AFMT_S16_LE ;
    else                                outgoing = AFMT_S16_LE ;
#endif /*UPSAMPLE*/
...
#if defined(UPSAMPLE)
    if ( true == peasycap->microphone ) incoming = 1;
    else                                incoming = 1;
#else
    if ( true == peasycap->microphone ) incoming = 0;
    else                                incoming = 1;
#endif /*UPSAMPLE*/
...
#if defined(UPSAMPLE)
    if ( true == peasycap->microphone ) incoming = 32000;
    else                                incoming = 48000;
#else
    if ( true == peasycap->microphone ) incoming =  8000;
    else                                incoming = 48000;
#endif /*UPSAMPLE*/
  ...
  case SNDCTL_DSP_SETTRIGGER:
    ...
    if ( 0 != copy_from_user(&incoming, (void __user *)arg, \
                                sizeof(int))) return(-EFAULT);
    JOT(8,"........... %i=incoming\n", incoming );
    JOT(8,"........... cf 0x%x=PCM_ENABLE_INPUT  0x%x=PCM_ENABLE_OUTPUT\n", \
...
int k;
/*-----------------------------------------------------------------------------
THE DATA FOR THE ARRAY mess BELOW WAS CONSTRUCTED BY MEANS OF A SHELL SCRIPT:
#
...
exit 0
#
AND THE EXCISED "_OLD" CASES WERE LATER REINSTATED MANUALLY.

THE DATA FOR THE ARRAY mess1 BELOW WAS CONSTRUCTED BY MEANS OF A SHELL SCRIPT:
