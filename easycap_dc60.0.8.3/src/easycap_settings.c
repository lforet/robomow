  ...
  for ( j = 0; j < RESOLUTION_MANY; j++ )
    ...
    for ( k = 0; k < PIXELFORMAT_MANY; k++ )
      ...
      for ( m = 0; m < INTERLACE_MANY; m++ )
        ...
        easycap_format[n].mask = mask1 | mask2 | mask3 | mask4;
        easycap_format[n].v4l2_format.type = V4L2_BUF_TYPE_VIDEO_CAPTURE,
        easycap_format[n].v4l2_format.fmt.pix.width = width;
        easycap_format[n].v4l2_format.fmt.pix.height = height;
        easycap_format[n].v4l2_format.fmt.pix.pixelformat = pixelformat;
        easycap_format[n].v4l2_format.fmt.pix.field = field;
        easycap_format[n].v4l2_format.fmt.pix.bytesperline = bytesperline;
        easycap_format[n].v4l2_format.fmt.pix.sizeimage = sizeimage;
        easycap_format[n].v4l2_format.fmt.pix.colorspace = colorspace;
        easycap_format[n].v4l2_format.fmt.pix.priv = 0;
        n++;
        }
      }
    }
