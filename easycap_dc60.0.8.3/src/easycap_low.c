...
if ( 0x80 == get2 ) { JOT(8,"confirm_stream:  OK\n"); }
...
while ( 0xFF != saa7113config[i0].reg )
  {
  if ( 0x0F == saa7113config[i0].reg ) { i0++; continue; }

  ir = read_saa(p, saa7113config[i0].reg);
  if ( ir != saa7113config[i0].set )
    {
    JOT(8,"SAA register 0x%02X has 0x%02X, expected 0x%02X\n", \
...
if ( max == j ) return(-1);
  ...
  {
  if ( 0x20 & rc ) JOT(8,"hardware detects 60 Hz\n");
...
while ( 0xFFF != stk1160config[i0].reg )
  {
  if ( 0x000 == stk1160config[i0].reg ) { i0++; continue; }
  if ( 0x002 == stk1160config[i0].reg ) { i0++; continue; }
  ...
  if ( 0x100 == stk1160config[i0].reg )
    ...
    if (( ir != ( 0xFF & stk1160config[i0].set )) && \
      ...
      {
      JOT(8,"STK register 0x%03X has 0x%02X, expected 0x%02X\n", \
                stk1160config[i0].reg, ir, stk1160config[i0].set );
      }
    i0++; continue;
    }
    ...
    switch (mode)
      ...
      case 7:
        ...
        if (0 != write_saa(p, 0x05, 0xFF))
          {
          SAY("ERROR: failed to set SAA register 0x05 for input %i\n", input);
          }
        break;
        }
      case 9:
...
ir = read_stk(p, 0x00);  JOT(8,"STK register 0x00 has 0x%02X\n", ir );
ir = read_saa(p, 0x02);  JOT(8,"SAA register 0x02 has 0x%02X\n", ir );
...
GET(p,0x0116,&igot116);  get116=igot116;
GET(p,0x0117,&igot117);  get117=igot117;
SET(p,0x0116,0x0000);
SET(p,0x0117,0x0000);
...
for ( k = 0;  k < max;  k++ )
  ...
  switch ( get0 )
    ...
    case 0x04 :
    case 0x01 : { return(0); }
    case 0x00 : { msleep(10); continue; }
    default:    { return(get0-1); }
    }
  }
return(-1);
}
...
if ( NULL == peasycap ) return(-EFAULT);

pusb_device = peasycap->pusb_device;
if (NULL == pusb_device) return(-EFAULT);
 ...
 *  THERE IS NO SEPARATE AUDIO CHIP PHYSICALLY PRESENT.
*/
/*---------------------------------------------------------------------------*/
...
if ( 0 > igot ) { SAY("ERROR: failed to read VT1612A register 0x02\n"); }
if ( 0x8000 & igot ) { JOT(8,"register 0x%02X muted\n", 0x02 ); }

igot = read_vt(pusb_device,0x000E);
if ( 0 > igot ) { SAY("ERROR: failed to read VT1612A register 0x0E\n"); }
if ( 0x8000 & igot ) { JOT(8,"register 0x%02X muted\n", 0x0E ); }

igot = read_vt(pusb_device,0x0010);
if ( 0 > igot ) { SAY("ERROR: failed to read VT1612A register 0x10\n"); }
if ( 0x8000 & igot ) { JOT(8,"register 0x%02X muted\n", 0x10 ); }

igot =read_vt(pusb_device,0x0012);
if ( 0 > igot ) { SAY("ERROR: failed to read VT1612A register 0x12\n"); }
if ( 0x8000 & igot ) { JOT(8,"register 0x%02X muted\n", 0x12 ); }

igot =read_vt(pusb_device,0x0016);
if ( 0 > igot ) { SAY("ERROR: failed to read VT1612A register 0x16\n"); }
if ( 0x8000 & igot ) { JOT(8,"register 0x%02X muted\n", 0x16 ); }

igot = read_vt(pusb_device,0x001A);
if ( 0 > igot ) { SAY("ERROR: failed to read VT1612A register 0x1A\n"); }
if ( 0x8000 & igot ) { JOT(8,"register 0x%02X muted\n", 0x1A ); }

igot = read_vt(pusb_device,0x001C);
if ( 0 > igot ) { SAY("ERROR: failed to read VT1612A register 0x1C\n"); }
if ( 0x8000 & igot ) { JOT(8,"register 0x%02X muted\n", 0x1C ); }
...
else mute = 0x8000 & ((unsigned int)igot);
...
if ( 0 > ( ir = regset(p, 0x0002, 0x0078) ))
  {
...
if ( 0 > ( ir = regset(p, 0x0002, 0x0093) ))
  {
  SAY("ERROR: failed to set register 0x0002 to 0x0078\n");
  }
return(ir);
