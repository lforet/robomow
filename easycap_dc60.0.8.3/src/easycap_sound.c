...
if ( ! ( pusb_interface = usb_find_interface(&easycap_usb_driver, subminor) ))
  {
  SAY("ERROR: pusb_interface is NULL\n");
  SAY("ending unsuccessfully\n");
  return(-1);
  }
if ( ! ( peasycap = usb_get_intfdata(pusb_interface) ))
  {
  SAY("ERROR: peasycap is NULL\n");
  SAY("ending unsuccessfully\n");
  return(-1);
...
if ( 0 <= rc ) { JOT(8,"audio_setup() returned %i\n",rc); }
else    { JOT(8,"easysnd open(): ERROR: audio_setup() returned %i\n",rc); }
...
if ( 0 != ( rc = adjust_volume( peasycap, -8192 )))
  {
...
if ( 0 == rc ) { JOT(8,"wakeup_device() returned %i\n",rc); }
else    { JOT(8,"easysnd open(): ERROR: wakeup_device() returned %i\n",rc); }
...
if ( file->f_flags & O_NONBLOCK )
  {
  JOT(16,"===== easysnd_read(): kount=%i, *poff=%i\n",(int)kount,(int)(*poff));
  }
else
  {
...
while (( fragment == \
  ...
  if ( 0 != ( wait_event_interruptible( peasycap->wq_audio, \
      (  peasycap->audio_idle  || \
         peasycap->audio_eof   || \
      (( fragment != \
         (peasycap->audio_fill / peasycap->audio_pages_per_fragment) ) && \
...
while ( fragment == \
  ...
  if ( ! kount1 )
    ...
    if ( fragment != \
          ( peasycap->audio_read / peasycap->audio_pages_per_fragment)) break;
  ...
  more = kount1;  if ( more > kount ) more = kount;
...
/*---------------------------------------------------------------------------*/
  if ( 0 != (rc = copy_to_user( puserspacebuffer, pdata_buffer->pto, more )))
    {
...
if ( ! peasycap->timeval1.tv_sec )
  ...
  if ( below ) sdr = signed_div( above, below );
  else         sdr.quotient = 192000;
  }
