...
#define EASYCAP_DRIVER_VERSION "0.8.3"
...
#define SETTINGS_MANY (STANDARD_MANY * RESOLUTION_MANY * 2 * \
                       PIXELFORMAT_MANY * INTERLACE_MANY)
/*-----------------------------------------------------------------------------
...
/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
  ...
  struct semaphore sem_mmap_video[FRAME_BUFFER_MANY];
  struct semaphore sem_timeval0;
  struct semaphore sem_timeval1;
...
-----------------------------------------------------------------------------*/
  ...
  int frame_fill;    /* Frame buffer being filled by field2frame().          */
                     /*   Bumped only by easycap_dqbuf() when field2frame()  */
                     /*   has created a complete frame.                      */
...
int              redaub(struct easycap *,void *,void *,int,int,__u8,__u8,bool);
...
int              adjust_format(struct easycap *,__u32,__u32,__u32,int,bool);
...
ssize_t          easysnd_read(struct file *, char __user *, \
                                            size_t, loff_t *);
...
int              set_resolution(struct usb_device *,__u16,__u16,__u16,__u16);
...
#define MICROSECONDS(X,Y) ((1000000*((long long int)(X.tv_sec - Y.tv_sec))) + \
...
do                                                       \
  ...
  if ( 0x8000 & u2 ) s = -(long int)( 0x7FFF & (~u2) );  \
  else               s =  (long int)( 0x7FFF & u2 );     \
  *(( X )) += (long int) 1;                              \
  *(( Y )) += (long int) s;                              \
