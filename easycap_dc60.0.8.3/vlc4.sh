...
if [ "x" = "x$1" ]; then
  echo "Usage:  ./vlc4.sh PAL"
  echo "or      ./vlc4.sh NTSC"
  exit 1
else
  standard="$1"
fi
...
if [ "PAL" = "${standard}" ]; then
  ${VLC} -vvv v4l2://${DEV_VIDEO}:width=720:height=576 :norm=pal :input-slave=oss://${DEV_AUDIO} --oss-samplerate=32000 --demux rawvideo 2>vlc.err
else
  ${VLC} -vvv v4l2://${DEV_VIDEO}:width=720:height=480 :norm=ntsc :input-slave=oss://${DEV_AUDIO} --oss-samplerate=32000 --demux rawvideo 2>vlc.err
fi
