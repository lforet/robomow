...
#
#-----------------------------------------------------------------------------
#   IMPORTANT:  IF SOUND IS BAD OR ABSENT CHANGE THE FOLLOWING LINE TO
#   RATE=32000
#-----------------------------------------------------------------------------
RATE=48000
...
if [ "x" = "x$1" ]; then
  echo "Usage:  ./tvtime.sh PAL"
  echo "or      ./tvtime.sh NTSC"
  exit 1
else
  standard="$1"
fi
...
do
  sleep 0.5
  tvtime-command run_command "(sox -c 2 -r ${RATE} -t raw -s2 ${DEV_AUDIO} -c 2 -r ${RATE} -s2 -t ossdsp /dev/dsp 1>/dev/null 2>/dev/null)" 1>/dev/null
  rc=$?
done
